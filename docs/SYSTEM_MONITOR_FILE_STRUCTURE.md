...
├── system_monitor/                    # 시스템 모니터링 애플리케이션
│   ├── __init__.py                    # Python 패키지 선언
│   ├── app.py                         # Flask 앱 조립, 화면·API 라우트
│   ├── config.py                      # 환경변수와 ROS 인터페이스 설정
│   ├── database.py                    # SQLite 스키마와 저장·검색
...
| `system_monitor/database.py` | 사용자·탐지·사건·트랩 테이블 생성과 CRUD | SQLite 연결을 요청 단위로 열고 커밋·롤백·닫기를 보장 |
...
| `system_monitor/security.py` | PBKDF2 비밀번호 해시 생성·검증 | 평문 비밀번호를 DB에 저장하지 않음 |
...
| 파일 | 역할 |
|---|---|
| `system_monitor/templates/login.html` | 관리자 로그인 폼 |
| `system_monitor/templates/dashboard.html` | KPI, 로봇 카드, 지도, 카메라, 타임라인, 제어 모달의 HTML 구조 |
| `system_monitor/templates/detections.html` | 탐지 조건 검색, 결과 목록, 검토 상태·메모 입력 구조 |
| `system_monitor/static/css/dashboard.css` | 로그인·관제·탐지 화면의 공통 색상, 크기, 반응형 배치, 모달 스타일 |
| `system_monitor/static/js/dashboard.js` | `/api/snapshot` 폴링, 상태 카드·카메라·지도 렌더링, 지도 마커 상세, 로봇 명령·확인 처리 |
| `system_monitor/static/js/detections.js` | 탐지 API 검색, 테이블 렌더링, 검토 상태와 메모 저장 |
| `system_monitor/static/favicon.svg` | WPC Control 웹 아이콘 |

## 6. 테스트 파일

| 파일 | 검증 내용 |
|---|---|
| `tests/test_app.py` | 로그인, 인증 API, 백그라운드 서비스 시작, Mock/ROS 응답 구조와 공통 화면 계약 |
| `tests/test_database.py` | 초기 관리자, 탐지 CRUD, 날짜 검색, 위험신호 정규화, 트랩 위치 저장 |
