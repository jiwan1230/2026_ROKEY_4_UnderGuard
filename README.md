...
| 노드 | 역할 | 실행 위치 |
|------|------|-----------|
| `camera_node` | rgb+depth sync 재발행 | 각 로봇 PC |
| `detector_node` | opening·rat 감지 → 검증/추적 target_pose 발행 | 각 로봇 PC |
...
| `db_node` | 구멍 좌표 DB (`/db/query_hole`) | 중앙 PC |
...
```bash
ros2 run turtle_project camera_node     --ros-args -r __ns:=/robot6
