    ...
    def _on_battery(self, robot_id: str, msg: Any) -> None:
        ...
        with self._data_lock:
            if (
                ...
                # 임계값 밑으로 처음 떨어진 순간에만 경고 이벤트를 한 번 기록한다.
                # 이후 값이 계속 낮아도 매 배터리 메시지마다 다시 경고를
                # 쌓지 않기 위해 robot_id를 "이미 보고함" 집합에 넣어 막는다.
            ...
            # 임계값 근처에서 값이 오르내리며 경고가 반복 발생(채터링)하지
            # 않도록, 임계값보다 2%p 더 높이 회복했을 때만 재무장한다.
