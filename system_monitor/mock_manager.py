...
class MockManager:
    ...
    def __init__(
        ...
        *,
        ...
        self._tick = 0
        # robot_id -> 마지막으로 적용된 수동 명령/이벤트 이름.
        # None이면 "자동 순찰"(_move_on_patrol) 중이라는 뜻이고,
        # PAUSE/STOP/TRAP_INSTALLED/TARGET_LOST 등이 들어 있으면
        # _update_robot_tick이 자동 이동을 건너뛰고 그 상태를 유지한다.
    ...
    def _move_on_patrol(
        ...
        # 실제 SLAM 이동 대신, tick이 지날수록 각도가 커지는 원운동
        # (cos/sin)으로 로봇이 맵 위를 순찰하는 것처럼 흉내 낸다.
        # index를 위상에 더해 로봇마다 다른 지점에서 출발하게 한다.
    ...
    def _trigger_scheduled_detections(self) -> None:
        ...
        if self._mission_progress >= 100:
            return
        # 아래 tick 번호(5·18·31, 12·26, 22)는 1초 = 1tick 기준으로
        # "쥐 최초 발견 → 역할 배정 → 쥐구멍/배설물 탐색" 시연 순서가
        # 자연스럽게 보이도록 임의로 정한 데모용 타임라인이다.
        # 정확한 물리 계산이 아니라 화면 시연 목적의 스크립트다.
