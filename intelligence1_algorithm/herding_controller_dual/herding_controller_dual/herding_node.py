...
_PARAM_DEFAULTS = {
    ...
    # --- 최소 이격 거리 (Driver/Blocker 목표점 간) ---
    "min_robot_separation_m": 0.6,
    # --- Occlusion 그리드 ---
    "diffusion_rate": 0.2,
    "decay_factor": 0.9,
    # --- 동적 Driver/Blocker 역할배정 (플랜 B 전용, config/herding_params.yaml 참고) ---
...
class HerdingNode(Node):
    ...
    def __init__(self) -> None:
        ...
        # (플랜 B) 두 로봇 모두 이 노드가 계산한 목표점을 따른다 -- 어느 쪽이
        # 이번 주기에 Driver/Blocker인지는 HerdingCore가 매 주기 다시
        # 판단하므로, robot1_goal/robot2_goal 둘 다 발행한다.
