...
class HerdingCore:
    ...
    때는 이 클래스가 실시간으로 Driver/Blocker를 다시 정한다. (플랜 A인
    `herding_controller`는 반대로 역할을 에피소드 내내 고정한다 -- 실제
    운용 제약 때문에 로봇 A는 이 패키지가 조종하지 않기로 한 결정이 있었고,
    거기서는 여전히 유효하다. 자세한 배경은
    herding_controller_트러블슈팅_노트.md 10번 항목 참고.)
    """
