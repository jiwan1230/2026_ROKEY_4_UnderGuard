# herding_controller_dual/test/test_role_assigner.py
import numpy as np

from herding_controller_dual.role_assigner import RoleAssigner, RoleAssignerConfig, resolve_separation


def make_role_config(margin=0.5, cooldown=2.0, turn_weight=0.3):
    return RoleAssignerConfig(
        role_swap_margin=margin, role_swap_cooldown_sec=cooldown, role_cost_turn_weight=turn_weight,
    )


def test_closer_robot_is_assigned_driver_initially():
    assigner = RoleAssigner(make_role_config())
...
def test_farther_robot1_is_not_bootstrapped_as_driver_on_first_call():
    ...
    assigner = RoleAssigner(make_role_config(margin=0.5, cooldown=2.0))
    driving_point = np.array([5.0, 5.0])
...
def test_role_does_not_swap_within_cooldown():
    assigner = RoleAssigner(make_role_config(margin=0.01, cooldown=2.0))
    driving_point = np.array([5.0, 5.0])
    heading = np.array([1.0, 0.0])
...
def test_role_cost_turn_weight_breaks_ties_between_equidistant_robots():
    ...
    assigner = RoleAssigner(make_role_config(margin=0.0001, cooldown=0.0))
    ...
    tie_assigner = RoleAssigner(make_role_config(margin=0.0001, cooldown=0.0, turn_weight=0.0))
