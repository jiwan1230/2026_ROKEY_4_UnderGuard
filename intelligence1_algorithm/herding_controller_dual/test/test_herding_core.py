...
def make_config(**overrides):
    ...
    defaults = dict(
        ...
        role_swap_margin=0.5, role_swap_cooldown_sec=2.0, role_cost_turn_weight=0.3,
    )
...
def test_off_grid_target_does_not_crash_the_cycle():
    ...
    assert output.escape_top3 == []
    # Runner 기본 위치(r1=(0,0), r2=(4,4))와 표적(50,50) 기준으로는 로봇 2가
    # 비용상 더 나은 Driver 후보로 뽑힌다. Driver에 대한 참고 지점은
    # grid_map과 무관하게 순수 기하학으로 계속 계산되며; blocking point는
    # 계산할 수 없으므로 blocker(로봇 1)는 자기 자리를 유지한다.
...
def test_no_module_in_the_core_import_chain_imports_rclpy():
    ...
    for name, module in list(sys.modules.items()):
        if not name.startswith("herding_controller_dual."):
            continue
