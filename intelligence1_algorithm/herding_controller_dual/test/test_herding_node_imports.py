# herding_controller_dual/test/test_herding_node_imports.py
...
`herding_controller_dual.herding_node`가 이미 sys.modules에 존재한다.
...
import herding_controller_dual.herding_node as herding_node
from herding_controller_dual.grid_map import GridConfig, GridMap
from herding_controller_dual.herding_core import HerdingConfig, HerdingOutput
from herding_controller_dual.state_machine import FSMState


def test_herding_node_is_the_only_module_importing_rclpy():
    package_dir = pathlib.Path(__file__).resolve().parent.parent / "herding_controller_dual"
    offenders = []
...
def _load_shipping_yaml_params() -> dict:
    ...
    with open(path) as handle:
        return yaml.safe_load(handle)["herding_controller_dual"]["ros__parameters"]
...
def test_param_defaults_match_shipping_yaml_values():
    ...
    herding_controller_dual herding_node`가 약 2.5% 성공률로 측정된 구성을
    조용히 실행하고 있었다.
    """
