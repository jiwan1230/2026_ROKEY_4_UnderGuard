...
# 실측 교체 (2026-08-09): 처음엔 포위도가 가장 높은 구석을 골랐는데,
# 60회씩 돌려보니 구석일수록 오히려 나빴다. 로봇이 좁은 데서 표적에 바짝
# 붙어야 해 panic이 폭증하고(top 43.3%), 협공도 안 선다.
#   (-0.23, 0.72) 성공 86.7% panic 43.3%  ->  (-1.28,-0.63) 성공 91.7% panic 5.0%
#   ( 1.57,-4.38) 성공 71.7% panic  1.7%  ->  ( 0.67,-2.83) 성공 86.7% panic 5.0%
#   (-2.28,-1.48) 성공 95.0% panic  3.3%  ->  유지
# 포위도는 "몰아넣기 쉬움"의 대리 지표였을 뿐이고, 실제로는 최종 성공률로
# 골라야 한다 -- 장애물 게이트 배치 때와 같은 교훈(트러블슈팅 노트 16-9).
TRAPS = {
    # 상단 팔 아래쪽, 벽까지 0.59m. 성공 91.7% / panic 5.0%
    "top": np.array([-1.28, -0.63]),
    # 좌측 벽 오목한 곳, 벽까지 0.15m. 성공 95.0% / panic 3.3%
    "left": np.array([-2.28, -1.48]),
    # 중앙 우측, 벽까지 0.20m. 성공 86.7% / panic 5.0%
    "bottom": np.array([0.67, -2.83]),
}
