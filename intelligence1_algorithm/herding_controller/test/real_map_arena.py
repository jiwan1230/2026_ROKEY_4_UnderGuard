# herding_controller/test/real_map_arena.py
...
# 대기 지점(충전소)은 옛 위치를 그대로 옮기면 안 된다. 옛 위치를 변환하면
# (-2.15,-0.40)인데, 로봇 파트의 실제 순찰 경로(36점)가 그 지점에서 7.8cm
# 거리로 지나간다 -- 두 로봇이 스쳐 지나가려면 중심 간 2*ROBOT_RADIUS_M
# (0.35m)이 필요하므로, 거기 주차된 로봇 B가 순찰을 물리적으로 막는다.
# 실측: 기본 표적 성공률 37.5%(막힘) vs 91.7%(경로 밖). 옛 맵에서는 우리가
# 임의로 잡은 5점 경로라 우연히 안 겹쳤을 뿐이다.
#
# 그래서 벽 여유를 만족하는 지점 중 순찰 경로에서 가장 먼 곳(0.33m)을 쓴다.
# 실기 주의: 이 방은 순찰 경로가 촘촘해 0.35m 이상 떨어진 자리가 아예 없다.
# 충전소를 순찰 경로에서 떨어뜨리거나, 순찰 경로가 충전소를 피해 가도록
# 로봇 파트와 조정해야 한다.
...
# 재-SLAM은 방을 다시 그린 게 아니라 좌표 원점만 옮긴 것이었다 (2026-08-09,
# 로봇 파트 확인 + 벽 상호상관 정합으로 검증: 최적 평행이동 (행 0, 열 +2)에서
# 벽이 거의 완전히 겹친다. IoU가 0.54로 낮게 나오는 건 벽이 1~2픽셀로 얇아
# 1픽셀만 어긋나도 반토막 나기 때문이지 방이 달라서가 아니다).
#
# 따라서 옛 맵에서 이미 검증된(93.7%) 덫 자리를 그대로 새 좌표계로 옮긴다:
#     새 = 옛 + (+0.61, +2.95) m
# 한때 포위도 기준으로 새로 골라봤지만 실측이 더 나빴다 -- 50회씩 비교에서
# 기본 94.7% vs 90.0%, 사람조작 98.7% vs 96.0%로 옮긴 자리가 이겼다.
# 같은 물리적 위치이니 당연한 결과다.
TRAPS = {
    "top": np.array([-2.20, -2.41]),
    "left": np.array([-1.56, 0.74]),
    "bottom": np.array([2.35, -3.41]),
}
