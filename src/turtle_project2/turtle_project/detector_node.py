...
class DetectorNode(Node):
    ...
    def __init__(self):
        ...
        # target_pose로 직접 쫓는다. 플랜 B: robot A도 알고리즘(herding_dual)이
        # 몰기 때문에 detector가 동시에 target_pose를 쏘면 방향이 반대라 로봇이
        # 진동한다 — plan=='b'면 추적 중 target_pose 발행을 끄고 쥐 위치
        # (rat_detected)만 계속 흘려 알고리즘 입력으로 쓰게 한다.
        # central_pc.launch.py의 plan:=a/b와 맞출 것.
    ...
    def _detect_rat(self, result, img_shape, depth, depth_frame):
        ...
        if due:
            # 몰이 알고리즘의 쥐 위치 입력 — 추적 시작 후에도 끊기면 안 된다(플랜 A/B 공통).
            self.event_pub.publish(String(data=fleet_msg.event(
                'rat_detected', *xy)))
...
def _self_check():
    ...
    # already_done: 이미 확인한 구멍 재점검 억제 (순찰 진행)
    assert already_done((0.0, 0.0), [], 0.5) is False                       # 목록 비었음
