...
> 알고리즘 자체는 시뮬레이션 성공률 93.7%까지 검증 완료(로봇 2대 협공 포함,
> 실물 검증은 아직). 해당 행만 정정하고 나머지는 재조사하지 않았다.
...
| 쥐몰이·goal | [rat_herding_node.py:36-44](../turtle_project/rat_herding_node.py#L36-L44) | **done**(정정, 08-09) | goal_cb가 herding_controller의 robot2_goal을 실제 target_pose로 relay — 알고리즘 쪽 몰이 로직 연결·시뮬 검증(93.7%) 완료. 실물 검증은 아직 |
